(ns tier_mobility.main
  "Kotodama WASM entrypoint for the Tier_mobility clean-room actor (L4) — Clojure port.
  ...
  facts (`tier_mobility.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tier_mobility")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "tiermobi_dri"
    :fields [:name :licensePlate :rating :online]              :required [:name :licensePlate]
    :coerce {:rating :float :online :bool}                       :refs {}}
   {:entity "Rider"      :plural "riders"      :id-prefix "tiermobi_rid"
    :fields [:name :phone :rating] :required [:name :phone]
    :coerce {:rating :float}          :refs {}}
   {:entity "Trip"     :plural "trips"     :id-prefix "tiermobi_tri"
    :fields [:riderId :driverId :origin :destination :status]        :required [:origin :destination]
    :coerce {} :refs {:riderId "Rider" :driverId "Driver"}}
   {:entity "Vehicle" :plural "vehicles" :id-prefix "tiermobi_veh"
    :fields [:driverId :make :model :plate]          :required [:make :model]
    :coerce {}                       :refs {:driverId "Driver"}}
   {:entity "Fare"   :plural "fares"   :id-prefix "tiermobi_far"
    :fields [:tripId :base :distanceKm :total :currency]   :required [:base :distanceKm]
    :coerce {:base :float :distanceKm :float :total :float}        :refs {:tripId "Trip"}}
   {:entity "Rating"    :plural "ratings"    :id-prefix "tiermobi_rat"
    :fields [:tripId :byRole :score :comment]  :required [:byRole :score]
    :coerce {:score :int}          :refs {:tripId "Trip"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"tier_mobility.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tier_mobility-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tier_mobility-compat/ready)
